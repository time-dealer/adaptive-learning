...
- 已完成：从公开 GitHub 仓库安装到独立临时项目；Skill 资源文件与发布包一致（不计本机 Finder 生成且未发布的 .DS_Store）。仓库 owner 后续改为 `KairoRogue`，新安装命令使用 `KairoRogue/adaptive-learning`，Skill id 后续改为 `rogue-learning`。
