...
本仓库的 Skill 本体是 Markdown 规则文件，用户安装和使用 Rogue Learning Skill 不需要 Python 依赖；`adaptive-learning` 是保留给安装器和显式调用的稳定 Skill id。

维护者在修改 Skill 后，可运行 Codex `skill-creator` 自带的结构校验脚本：

```bash
python /Users/dealer/.codex/skills/.system/skill-creator/scripts/quick_validate.py skills/adaptive-learning
```
...
- `quick_validate.py skills/adaptive-learning` 能通过；
